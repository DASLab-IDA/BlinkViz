...
class BinaryTreeConv(nn.Module):
    ...
    def forward(self, flat_data):
        ...
        #trees = trees.to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
        #idxes = idxes.to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
        orig_idxes = idxes
        ...
        zero_vec = zero_vec.to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
        #results = results.to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
