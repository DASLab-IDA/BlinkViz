...
def cluster_center_update_dataset_batch(spn, dataset):
    """
    Updates the SPN when a new batch of dataset arrives. Extend the 'cluster_center_update_dataset()' 
    from 1-D to m-D
    """
    if isinstance(spn, Categorical):
        ...
        adapt_weights_batch(spn, min_idx)
        children_to_be_update = np.unique(min_idx)
...
def projection_batch(dataset, scope):
    projection = []
    for item in dataset:
        proj = []
        for idx in scope:
            assert len(item) > idx, "wrong scope " + str(scope) + " for dataset" + str(item)
            proj.append(item[idx])
...
def insert_into_categorical_leaf_batch(node, data, insert_weights, debug=False):
    """
    Updates categorical leaf according to a batch of data with associated insert_weights
    """
